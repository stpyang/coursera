...
class Empty extends TweetSet {
  ...
  override def union(other: TweetSet): TweetSet = other
  
  override def mostRetweeted = throw new NoSuchElementException

  override def mostRetweetedAcc(that: Tweet): Tweet = that
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweetedAcc(that: Tweet): Tweet =
    if (elem.retweets > that.retweets) right.mostRetweetedAcc(left.mostRetweetedAcc(elem))
    else right.mostRetweetedAcc(left.mostRetweetedAcc(that))
